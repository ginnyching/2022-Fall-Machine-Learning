...
Original file is located at
    https://colab.research.google.com/drive/1nwTJpcvvXziWVkk0fv7cfoY5zLQljVH8
"""
...
# y_train not necessary


def cross_validation(x_train, y_train, k=5):
    kfold_data = []
    # if size of x_train is not divisible by k,
    # the first len(x_train)%k folds will have size len(x_train)//k+1
    # the rest folds have size len(x_train)//k
...
for i in range(len(kfold_data)):
    print("Split: ", i+1, ", Training index: ",
          kfold_data[i][0], ", Validation index: ", kfold_data[i][1])
...
for kernel in ['rbf']:
    for C in c_list:
        validate_acc = []
        # train the model for every possible parameter combination (7*8)
...
# plot the heatmap of grid search result of SVM
...
draw_heatmap(gamma_list, c_list, accuracy_data)
...
best_model = SVC(
    C=best_parameters["C"], kernel=best_parameters["kernel"], gamma=best_parameters["gamma"])
